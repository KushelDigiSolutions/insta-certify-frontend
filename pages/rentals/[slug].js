...
import style from "../../pages/css/rentalList.module.scss";
...
export default function RentalsLists(pageProp) {
  ...
  const loadMore = async () => {
    ...
    const reqFetch = await fetch(process.env.server.api+"get-lodging/"+endpoint);
    ...
    <div className={`page_rental_list ${style.pageRentalMain}`}>
      ...
      <div className={style.categoryLayoutFull}>
        <div className="container">
          ...
          :
          (<p className={style.notEvent}>There are no lodging in {lodgingCategory?.name?.toLowerCase()} category.</p>)
          }
...
export async function getServerSideProps(context) {
  ...
  if(urlSlug != ""){
    ...
    const dataFetch = await fetch(process.env.server.api + "get-lodging/"+urlSlug+filterStr);
